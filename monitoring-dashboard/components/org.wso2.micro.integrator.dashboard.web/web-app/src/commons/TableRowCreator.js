...
export default function TableRowCreator(props) {
    ...
    return <TableRow>
        {headers.map(header => {switch(header.id) {
            ...
            // Registry Resources
            case 'fileIcon':
                return <TableCell>{data.fileIcon}</TableCell>
            case 'childName':
                // return <TableCell>{data.childName}</TableCell>
                return <TableCell><IconCell pageId={pageId} name={data.childName} iconType={data.fileIcon} retrieveData={retrieveData} handleDoubleClick={props.handleDoubleClick} registryPath={props.registryPath} data={data}/></TableCell>
            case 'mediaType':
